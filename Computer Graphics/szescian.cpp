...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(windowWidth, windowHeight, "Sześcian 3D", nullptr, nullptr);
    if (!window)
    {
    ...
    // Ustawienie okna GLFW jako bieżącego kontekstu OpenGL
    glfwMakeContextCurrent(window);

    // Inicjalizacja GLAD
    if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress))
    {
        std::cerr << "Nie udało się zainicjować GLAD" << std::endl;
        glfwTerminate();
        return -1;
    }
    ...
    if (!status)
    {
        ...
        std::cout << "Błąd (Shader wierzchołków): " << error_message << std::endl;
    }
    ...
    if (!status)
    {
        ...
        std::cout << "Błąd (Shader fragmentów): " << error_message << std::endl;
    }
    ...
    if (!status)
    {
        ...
        std::cout << "Błąd (Program shaderów): " << error_message << std::endl;
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Wyczyszczenie buforów koloru i głębi
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

        // Użycie programu shaderów
        ...
        // Powiązanie VAO i narysowanie sześcianu
        glBindVertexArray(VAO);
